...
def save_labels(texts):
    ...
    with open(config.labels_path, 'wb') as fw:
        pickle.dump([texts,char2id, id2char], fw)

    return texts,char2id,id2char


def load_labels():
    with open(config.labels_path, 'rb') as f:  
        texts,char2id,id2char = pickle.load(f)
    return texts,char2id,id2char
...
def run():
    ...
    # texts,char2id,id2char = save_labels(texts)

    print("-----load labels-----")
    texts,char2id,id2char = load_labels()
    ...
    history = model.fit_generator(
        ...
        callbacks=[checkpointer, lr_decay,tf_callback])
